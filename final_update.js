...
async function downloadAndReplace() {
  ...
  // Fathers
  text = text.replace(/Mrs\.(?:\\x20|\s+)&(?:\\x20|\s+)Mr\.(?:\\x20|\s+)Dr\.(?:\\x20|\s+)Syed(?:\\x20|\s+)Shaker(?:\\x20|\s+)Arif/g, 'Mohammad Aarif');
  text = text.replace(/Mrs\.(?:\\x20|\s+)&(?:\\x20|\s+)Mr\.(?:\\x20|\s+)Syed(?:\\x20|\s+)Azam(?:\\x20|\s+)Sahab/g, 'Waqeel Khan');
